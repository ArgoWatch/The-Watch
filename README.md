...
- **Curate** (formerly Crew) sits with Play / Strip / Draw. Click Curate to show only the address field (Ossen’s as the example). **+** adds another wallet; **×** on a row drops that wallet (the last one resets). Curate flashes with a building ellipsis while holdings load; the strip fills as each Argo is confirmed. **−** hides Curate chrome; **×** leaves Curate but remembers the show — Curate toggles the chrome or restores the last show. **Suit Up** is simplest-to-most-worn; **Themes** ‹ › plays the next echo; **As Minted** is id order; **As Received** is first inbound per holding; drag (or two taps on a phone) the strip for As Arranged. × under a well leaves that Argo in the show. The same addresses are remembered in the browser for about 30 minutes. The title returns to the start of the current show.
