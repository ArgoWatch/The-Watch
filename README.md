...
- **Crew** sits with Play / Strip / Draw. Click Crew to show only the address field (Ossen’s as the example). **+** adds another wallet (the strip stays); **×** on a row drops that wallet (the last one resets). Pasting over an address rebuilds that crew from scratch rather than mixing the old strip with the new wallet. Crew flashes with a building ellipsis while holdings load; the strip fills as each Argo is confirmed. **−** hides Crew chrome; **×** leaves Crew but remembers the show — Crew toggles the chrome or restores the last crew. **Suit Up** is simplest-to-most-worn; **Themes** ‹ › plays the next echo; **As Minted** is id order; **As Received** is first inbound per holding; drag (or two taps on a phone) the strip for As Arranged. × under a well leaves that Argo in the show. The same addresses are remembered in the browser for about 30 minutes, and only when the pull matches `balanceOf` (incognito has no cache, so it always reads the chain). A copied Crew URL includes the token list (`hold=`) and arrival blocks (`recv=`), so the person opening it gets the same show — including As Received — even if their browser’s log scan is slower. The title returns to the start of the current show.
