...
- **Crew** sits with Play / Strip / Draw. Click Crew to show only the address field (Ossen’s as the example); **+** adds another. Crew flashes with a building ellipsis while holdings load; the strip fills as each Argo is confirmed. Then: **×** leaves Crew but remembers the show — Crew again brings it back. **+** under the strip reopens addresses; × on that row starts a new crew. **Suit Up** is simplest-to-most-worn; **As Minted** is id order; the echo rolodex is categories (Cloak, Sight, …) then specific names; **As Arrived**; drag the strip for As arranged. × under a well leaves that Argo in the show. The same addresses are remembered in the browser for about 30 minutes. The title returns to the start of the current show.
