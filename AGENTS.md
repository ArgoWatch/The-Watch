...
- Twins: same slots filled, differ in exactly one slot, and that slot is a subtle substitute (not None vs a value, not Palette/Bones/Cloak/Crown/Relic). Sight allowlist only: Chanel↔Prada, Shades↔Digital, Gucci↔Dior. Number is the 1px-offset door; the differing Sight cells pulse and are the same door. Fate has no door. Not Shades↔Glasses.
- Stem film: only true bare-bones Argos (Palette + Bones, every optional slot empty). Hover: paper/charcoal inner-edge pulse (reads on any palette), play-notch cursor immediately. Press anywhere on the plate starts at once and blurs the id box (mobile number pad dismisses). Extra clicks in the first beat are ignored. Stem is first and last frame. Caption and URL stay on the stem. A later click, leaving the tile (tracked by cursor position, not a finger lift), Escape, or Space stops and restores the still. No chrome, no timeline, no autoplay, no URL churn. Path is a consistent build of one household (Sight → Artifact → Relic → Crown, values once set do not change). If a cloaked counterpart exists, end on it — cloak last, matching the dressed slots as far as the collection allows (Crown and Cloak never co-occur; the cloak is the curtain). ~800ms per frame, longer hold on the cloak. Skip Fate so the ground never jumps. Number remains the link; play is extra.
