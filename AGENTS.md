...
- Stem film: only true bare-bones Argos (Palette + Bones, every optional slot empty). Hover: one 1px inner-edge pulse in catalog ink at ~10% opacity, then a play-notch cursor. Click plays the (Bones, Palette) household in place; stem is first and last frame. Caption and URL stay on the stem. Second click, leaving the tile (not a finger lift), Escape, or Space stops and restores the still. No chrome, no timeline, no autoplay, no URL churn. Order is constructive (Sight → Artifact → Relic → Cloak → Crown, one added slot at a time, common values first, Sight twins adjacent). Families larger than 36 use a representative path (stem → one single-add per slot → fullest coda). Skip Fate so the ground never jumps. Number remains the link; play is extra.
