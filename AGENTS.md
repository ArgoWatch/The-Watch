...
- Touch: tap empty canvas to show transport chrome for 3s; interacting with a control resets the timer. Sheet sits under the mode row in Draw. Fate (and other non-slot extras except Print) are inert, like None. Pull down from the top of the page (mast / top padding, not the plate) reloads. `overscroll-behavior: none` stays so a drag on the picture does not native-refresh.
