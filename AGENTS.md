...
- Watch speed is 0.5× / 1× / 2×. Default is 0.5×. 2× is the cap. 0.5× still beat is ~2.8s (stamp lamp in the middle, then a quiet print before the cut). Play keeps the current picture until the next SVG is ready — no veil dim, no 0.7s opacity fade between Argos. Fate/Burned adds ~0.8s. GIF `tokenURI` images mount as `<img>`. On-chain SMIL (`animate` on rects) is kept. Burned `tokenURI.image` is still SVG with no `<animate>` (#7135, 2026-09-12); OpenSea `animation_url` has the fire SMIL — do not fetch that CDN. No fate caption in Play.
