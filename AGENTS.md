...
- Isolation well is 2in square on desktop (half the 4in plate). On narrow screens, isolation is half the assembled scale and sits to the right of the plate list. None plates are not clickable. Dark isolations (mean luma under 0.4) use a paper well so black cloaks and eyepatches read; light isolations stay on black.
...
- **Draw** paints the current SVG rects in document order. While plotting, the Draw label reads **Pause**; press it to pause, again to resume. **Reset** restarts the plot. **Play** in Draw mode leaves Draw and resumes the slideshow. **Sheet** sits under the mode row in Draw and downloads a night-mode 24×24 graph-paper PDF (white field only; site colors elsewhere).
