...
- Touch: tap empty canvas to show transport chrome for 3s; interacting with a control resets the timer. Sheet sits under the mode row in Draw. Fate (and other non-slot extras except Print) are inert, like None. Pull down from above the plate (mast, padding, or a vertical drag that started on Play/Strip/Draw) reloads. The picture itself does not. Native overscroll-y is on so the browser’s own pull-to-refresh can fire too.
