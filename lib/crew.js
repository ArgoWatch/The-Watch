  ...
  async function holdings(addresses, opts) {
    ...
    function ownerOfRetry(id) {
      ...
      function once() {
        return TS.chain.ownerOf(id).catch(function (err) {
          attempt += 1;
          if (attempt >= 3 || aborted()) throw err;
          return new Promise(function (resolve) {
    ...
    async function scanBothEnds(addr, want, chunk, opts) {
      ...
      while (lo <= hi) {
        ...
        function takePack(pack) {
          const more = takeLogs(pack);
          for (let g = 0; g < more.length; g++) gaps.push(more[g]);
          return pack;
        }
        const jobs = [inboundChunk(addr, lo, loEnd, splitEmpty).then(takePack)];
        if (hiStart <= hi) jobs.push(inboundChunk(addr, hiStart, hi, splitEmpty).then(takePack));
