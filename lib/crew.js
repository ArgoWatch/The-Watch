(function (global) {
  ...
  function isFreshHold(rec, want, latest) {
    ...
    if (rec.complete === false) return false;
  ...
  async function holdings(addresses, opts) {
    ...
    for (let a = 0; a < addrs.length; a++) {
      ...
      while (lo <= hi) {
        ...
        const jobs = [inboundChunk(addr, lo, loEnd).then(eat)];
        if (hiStart <= hi) jobs.push(inboundChunk(addr, hiStart, hi).then(eat));
        await Promise.all(jobs);
        lo = loEnd + 1;
        hi = hiStart - 1;
        await pool.drain();
        if (want != null && countMine(addr) >= want) break;
      }
    }
    await pool.drain();
    ...
    for (let i = 0; i < addrs.length; i++) {
      ...
      if (want != null && mine.length !== want) complete = false;
      if (ok && mine.length) {
        await writeHold(addr, {
          ...
          toBlock: latest,
          fetchedAt: fetchedAt,
          complete: want == null || mine.length === want,
          want: want,
        });
      }
    }
