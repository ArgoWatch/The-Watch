(function (global) {
  ...
  async function inboundChunk(addr, start, end, splitEmpty) {
    const topics = [TRANSFER, null, padAddr(addr)];
    const span = end - start + 1;
    try {
      const logs = await logsInRange(start, end, topics);
      if (splitEmpty && (!logs || !logs.length) && span > 500) {
        const mid = Math.floor((start + end) / 2);
        if (mid > start) {
          const a = await inboundChunk(addr, start, mid, true);
          const b = await inboundChunk(addr, mid + 1, end, true);
          return {
      ...
      const a = await inboundChunk(addr, start, mid, splitEmpty);
      const b = await inboundChunk(addr, mid + 1, end, splitEmpty);
      return {
        logs: a.logs.concat(b.logs),
        gaps: a.gaps.concat(b.gaps),
      };
  ...
  async function holdings(addresses, opts) {
    ...
    for (let i = 0; i < addrs.length; i++) {
      ...
      cached[i] = rec;
      let want = null;
      for (let t = 0; t < 3 && want == null; t++) {
        try {
          if (TS.chain.balanceOf) {
            const n = await TS.chain.balanceOf(addrs[i]);
    ...
    function ownerOfRetry(id) {
      let attempt = 0;
      function once() {
        return TS.chain.ownerOf(id).catch(function (err) {
          attempt += 1;
          if (attempt >= 4 || aborted()) throw err;
          return new Promise(function (resolve) {
    ...
    function consider(id) {
      ...
      return pool.add(function () {
        ...
        return ownerOfRetry(id).then(function (owner) {
          if (aborted()) return;
    ...
    function idsMet(addr, want) {
      return want != null && countMine(addr) >= want;
    }

    async function scanBothEnds(addr, want, chunk, opts) {
      opts = opts || {};
      const splitEmpty = !!opts.splitEmpty;
      const untilFirstIn = !!opts.untilFirstIn;
      ...
      while (lo <= hi) {
        ...
        const jobs = [inboundChunk(addr, lo, loEnd, splitEmpty)];
        if (hiStart <= hi) jobs.push(inboundChunk(addr, hiStart, hi, splitEmpty));
        ...
        await pool.drain();
        noteIdsReady();
        if (idsMet(addr, want) && (!untilFirstIn || missingFirstIn(addr) === 0)) {
          gaps = [];
          break;
        }
      }
      if (gaps.length) await scanGaps(addr, gaps, want, splitEmpty, untilFirstIn);
      noteIdsReady();
    }

    async function scanGaps(addr, gaps, want, splitEmpty, untilFirstIn) {
      ...
      while (missed.length && tries < 3 && !aborted()) {
        ...
        for (let m = 0; m < again.length; m++) {
          if (aborted()) break;
          const pack = await inboundChunk(addr, again[m][0], again[m][1], splitEmpty);
          const more = takeLogs(pack);
          for (let g = 0; g < more.length; g++) missed.push(more[g]);
        }
        await pool.drain();
        if (idsMet(addr, want) && (!untilFirstIn || missingFirstIn(addr) === 0)) return [];
      }
      return missed;
    }

    async function fillWallet(addr, want) {
      await scanBothEnds(addr, want, CHUNK, { splitEmpty: true });
      if (idsMet(addr, want)) return;
      await scanBothEnds(addr, want, 2000, { splitEmpty: true });
      if (idsMet(addr, want)) return;
      await scanBothEnds(addr, want, 800, { splitEmpty: true });
    }
    ...
    for (let a = 0; a < addrs.length; a++) {
      ...
      await pool.drain();
      noteIdsReady();
      if (idsMet(addr, want)) continue;
      await fillWallet(addr, want);
    }
    await pool.drain();
    if (!aborted()) {
      let retry = 0;
    ...
    if (!aborted()) {
      let needIn = false;
      for (let i = 0; i < addrs.length; i++) {
        if (missingFirstIn(addrs[i])) needIn = true;
      }
      if (needIn) {
        ...
        for (let i = 0; i < addrs.length; i++) {
          if (aborted()) break;
          if (!missingFirstIn(addrs[i])) continue;
          await scanBothEnds(addrs[i], wants[i], CHUNK, { untilFirstIn: true });
        }
      }
    }
