(function (global) {
  ...
  function cellFill(c) {
    if (!c) return null;
    return typeof c === "string" ? c : c.fill;
  }
  ...
  function traitDiffCells(ta, tb) {
    ...
    const job = Promise.all([
      ...
      for (let y = 0; y < 24; y++) {
        for (let x = 0; x < 24; x++) {
          if (cellFill(a[y][x]) !== cellFill(b[y][x])) cells.push({ x: x, y: y });
        }
      }
      return cells;
    }).catch(function (err) {
  ...
  function gridToSvg(grid) {
    ...
    for (let y = 0; y < 24; y++) {
      for (let x = 0; x < 24; x++) {
        const fill = cellFill(grid[y][x]);
        if (fill && TS.decode.HEX.test(fill)) {
          const o = cellOp(grid[y][x]);
          const op = o < 0.999 ? ' fill-opacity="' + (Math.round(o * 1000) / 1000) + '"' : "";
          parts.push(
            '<rect x="' + x + '" y="' + y + '" width="1" height="1" fill="' + fill + '"' + op + "/>"
          );
        }
      }
    }
  ...
  function diffGrid(a, b) {
    ...
    for (let y = 0; y < 24; y++) {
      for (let x = 0; x < 24; x++) {
        if (cellFill(a[y][x]) !== cellFill(b[y][x])) {
          g[y][x] = a[y][x];
          if (cellFill(a[y][x])) n += 1;
        }
      }
    }
    return n ? g : null;
  ...
  function bind(host) {
    ...
    function isolateSlotFrom(slot, printG, fullRenderG, bareG, wornSvg, bg) {
      ...
      for (let y = 0; y < 24; y++) {
        for (let x = 0; x < 24; x++) {
          const layer = wornG[y][x];
          const belongs = slot === 1
            ? !!(cellFill(layer) && cellFill(layer) !== wornBg)
            : !!(bareG && cellFill(layer) !== cellFill(bareG[y][x]));
          if (!belongs) continue;
          const printC = printG && printG[y][x];
          const renderC = fullRenderG && fullRenderG[y][x];
          // Own construction, except where tokenURI rewrites a renderer pixel (Dragon's Breath tip).
          out[y][x] = (cellFill(printC) && cellFill(renderC) && cellFill(printC) !== cellFill(renderC))
            ? printC
            : layer;
          if (cellFill(out[y][x])) n += 1;
        }
      }
      return n ? gridToSvg(out) : "none";
    }

    async function loadChips(traits, fullSvg, opts) {
      const my = ++chipSeq;
      if (!traits || !TS.chain.renderTraits) return;
      ...
      const printG = gridFromSvg(fullSvg);
      ...
      const fullRenderG = gridFromSvg(byKey[comboKey(traits)] || "");
      for (let s = 1; s <= 6; s++) {
      ...
      const mask = opts && opts.fleeceMask;
      if (mask && mask.length && printG) {
        const fleeceG = emptyGrid();
        let n = 0;
        for (let i = 0; i < mask.length; i++) {
          const p = mask[i];
          const x = p % 24;
          const y = (p / 24) | 0;
          if (y < 0 || y > 23 || x < 0 || x > 23) continue;
          const c = printG[y][x];
          if (!cellFill(c)) continue;
          fleeceG[y][x] = c;
          n += 1;
        }
        if (n) isolations[6] = gridToSvg(fleeceG);
      }
      if (my !== chipSeq) return;
      emit();
    }
