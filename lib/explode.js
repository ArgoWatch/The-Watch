(function (global) {
  ...
  function gridFromSvg(svgText) {
    ...
    rects.forEach(function (r) {
      // ACK print stamps: 1×1 #000/#fff at fill-opacity ~0.11–0.14. Not trait paint.
      if (isPrintStamp(r)) return;
  ...
  const relicMemo = new Map();

  function relicCells(traits) {
    if (!traits || traits.length !== 7 || !TS.chain.renderTraits) {
      return Promise.resolve([]);
    }
    const on = Array.from(traits);
    on[3] = 1;
    const off = Array.from(traits);
    off[3] = 0;
    const key = on.join(",") + "|" + off.join(",");
    if (relicMemo.has(key)) return relicMemo.get(key);
    const job = Promise.all([
      TS.chain.renderTraits(on),
      TS.chain.renderTraits(off),
    ]).then(function (pair) {
      ...
      relicMemo.delete(key);
      throw err;
    });
    relicMemo.set(key, job);
    return job;
  }
  ...
  function bind(host) {
    ...
    async function isolateSlot(traits, slot, tokenG, bareG, bg) {
      ...
      const wornBgMatch = /fill="(#[0-9A-Fa-f]{3,6})"/.exec(pair[0] || "");
  ...
  TS.explode = { bind: bind, HARBOR: HARBOR, relicCells: relicCells };
})(window);
