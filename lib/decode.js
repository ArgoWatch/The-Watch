(function (global) {
  ...
  function metaFromTokenURI(uri) {
    ...
    const fleece = parseFleeceBreath(meta.animation_url, attributes);
    return {
      svg: img.svg,
  ...
  function parseFleeceBreath(animationUrl, attributes) {
    const html = decodeDataHtml(animationUrl);
    if (!html || html.indexOf("FLEECE") < 0) return null;
