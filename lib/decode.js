(function (global) {
  ...
  function frameIsFate(frame) {
    ...
    if (frame.kind === "gif" || frame.animated) return true;
  ...
  function needsPaperWell(svgText) {
    ...
    if (!n) return false;
    return sum / (n * 255) < 0.4;
  }

  const WELL = Object.create(null);
  const PAPER_NAME = /death|corsair|eye\s*patch|void|black|woodpipe|wood\s*pipe|\bpipe\b|shadow|obsidian/;
